...
UnrealProtocol.prototype.logon = function() {
    ...
    this.sendRaw("PROTOCTL EAUTH=services.YkBots.tk SID=9JS");
    this.sendRaw("PROTOCTL NOQUIT NICKv2 SJOIN SJ3 NICKIP TKLEXT TKLEXT2 CLK EXTSWHOIS MLOCK ESVID");
    this.sendRaw("SERVER services.YkBots.tk 1 : Servidor de Servicios de a red");
}
...
UnrealProtocol.prototype.introduceUser = function(nickname, ident, realname, hostname) {
    ...
    this.sendRaw(`UID ${nickname} 0 ${Math.floor(new Date().getTime() / 1000)} ${ident} ${hostname} UID 0 +iorKkBdH  ${hostname} ${hostname} * : ${realname}`);
}
